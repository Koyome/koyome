...
> 最后更新：2026-09-24（第二十二轮：暗态进入首页后人像滞留暗色素材——结构性修复）**本轮未推送，由用户一键脚本自行上线**
> 仓库状态：本地 HEAD = 本轮提交（见 `git log`）；**远端 main 仍在 `6a916d1`——R20（推送加固）、R21、R22（本轮）三轮改动均在本地，待用户一键脚本推送** ｜ ⚠️ 常驻推送授权**已于 2026-09-23 取消**：AI 推送前必须逐次征得用户同意（§4.3）

---

## 0.20 第二十二轮速览（2026-09-24）

- **首页人像暗色素材滞留亮态（用户报"关于页切夜间→回首页→切回日间，人像看不清且发蓝"）——根因与修复**：根因**不是滤镜/冲突**，是 R21 换图机制的缓存漏洞：index.html 人像的**内联首帧守卫**在暗态加载时把 `src` 抢先换成暗色版，但该 img 只有 `data-dark-src`、**没有 `data-light-src`**；随后 header.js 首扫 `swapCutouts()` 执行 `lightSrc ??= 当前 src`，把**已是暗色版的 src 缓存成"亮色版"**——之后切回日间 `img.src = lightSrc` 端出的仍是暗色素材（暗色素材是浅色线条烘焙给黑底用的，落在浅纸上 = 看不清+发蓝）。触发条件=**任意页面切暗后进入/刷新首页**（关于页只是用户切暗的地方）。修复双管齐下：
  - index.html 人像补 `data-light-src="assets/avatar_cutout.webp"`（HTML 解析期即就位，早于内联守卫与 header.js，缓存永远正确）；
  - header.js 加固：缓存 lightSrc 时**若当前 src 已等于 darkSrc 则拒绝缓存**（防同类内联守卫复发），亮态缺 lightSrc 时回退当前 src 而非 undefined。
  - 其余 3 张暗色素材不受影响：catalog/guestbook 是静态标记（src 恒为亮色版，首扫缓存正确）；entry.js t2 动态人像本就显式写 data-light-src。
  - **教训：任何"在 header.js 首扫之前改写 img.src 的首帧守卫"，必须同时在标记里显式写 data-light-src——守卫和缓存的时序耦合是隐性炸弹。**
- **验证**：新探针 `tools/probe-portrait-theme.js`（jsdom 复现完整链路：暗态加载首页→切亮→切暗→切亮，断言 lightSrc 缓存与每次 src，5/5 过）；全量 jsdom 回归 51/51 绿。**本轮代码未推送**（用户一键脚本自行上线，§4.3）。
