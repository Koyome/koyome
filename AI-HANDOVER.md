...
> 最后更新：2026-09-24（第二十二轮：人像主题换图两处缓存/覆盖漏洞修复 + 星空银河微光带）**本轮未推送，由用户一键脚本自行上线**
...
- **验证**：jsdom 回归扩至 **53 项**（新增暗态加载首页端到端：全脚本沉降后人像必须在暗色版、切亮必须回亮色版）全绿；新探针 `tools/probe-portrait-theme.js`（5/5）；Edge headless 实拍 t2 页 日/夜/移动 3 张目检通过（spec `tools/shot-spec-r22.json`，截图 `tools/shots-r22/`，临时服务器 `tools/_serve-docs.js`）。**本轮代码未推送**（用户一键脚本自行上线，§4.3）。
